...
export class CerebrasProvider extends BaseProvider implements AIProvider {
  ...
  async generateText(prompt: string, options?: RequestOptions): Promise<AIResponse> {
    ...
    if (!apiKey) {
      throw new ValidationError("Cerebras API key is not configured.", this.name);
    }
    ...
    if (contentText === undefined || contentText === null) {
      throw new ValidationError("Invalid or empty response from Cerebras API.", this.name);
    }
